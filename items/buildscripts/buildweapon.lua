...
--法杖主要技能名的元素部分
 ...
 then
   replacePatternInData(config, nil, "<elementalName>", "电弧")
 end
 if (elementalType=="poison")
 then
   replacePatternInData(config, nil, "<elementalName>", "剧毒")
 end
  ...
  if config.twoHanded then
    config.tooltipFields.handednessLabel = "双手"
  else
    config.tooltipFields.handednessLabel = "单手"
  end
